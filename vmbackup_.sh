...
for VM_ELEMENT in "${VM_LIST[@]}"; do
 ...
 while read VMID VMNAME; do 
  ...
  if ! [ -d $BAKUP_PATH/$VMID-$VMNAME ];
  then
   mkdir -p $BAKUP_PATH/$VMID-$VMNAME/$NOW;
  else
   mkdir $BAKUP_PATH/$VMID-$VMNAME/$NOW;
  fi
  ...
  onevm show $VMID --all -x > $BAKUP_PATH/$VMID-$VMNAME/$NOW/vm.xml;
  onevm show $VMID --all > $BAKUP_PATH/$VMID-$VMNAME/$NOW/vm.txt;
  ...
  while read IMAGE_ELEMENT;  do
    ...
    if [ $IMAGE_ELEMENT = "</DISK>" ]; then
      ...
      if [[ $PERSISTENT = "No" ]]; then
        ...
        DISK_LIST+=($RBD_POOL/$RBD_IMAGE@$RBD_IMAGE-$NOW+$BAKUP_PATH/$VMID-$VMNAME/$NOW/$RBD_IMAGE);
        ...
        DISK_LIST+=($RBD_IMAGE@$RBD_POOL-$IMAGE_ID-$NOW+$BAKUP_PATH/$VMID-$VMNAME/$NOW/$RBD_POOL-$IMAGE_ID);
	  else
        if [ $VERBOSE ]; then echo "["$(date +"%F-%T")"] $IMAGE_ID-$VMID not set as a persistent or non-persistent!!!"; fi
      fi
    fi

  done < $BAKUP_PATH/$VMID-$VMNAME/$NOW/vm.xml;
